...
`launch`, `async`의 `block`은 리시버 타입이 `CoroutineScope`인 함수형 타입이다. 부모 스코프에서 제공되는 리시버를 통해 호출될 수 있음을 의미한다. 부모는 자식들을 위한 스코프를 제공하고, 자식들을 해당 스코프 내에서 호출한다. 이를 통해 **구조화된 동시성**이라는 관계가 성립한다.
